...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        z, c_lq, z_gt, x, gt, yrec, xc = self.get_input(batch, self.first_stage_key,
                                           ...
                                           return_original_cond=True,
                                           bs=N, val=True)
        # x, c, gt, ref = self.get_input(batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        #           cond_key=None, return_original_cond=False, bs=None, val=False, text_cond=[''], return_gt=False, resize_lq=True):
        N = min(x.shape[0], N)
        n_row = min(x.shape[0], n_row)
        if self.test_gt:
...
class LatentDiffusionSRTextWTFFHQRef(LatentDiffusionSRTextWT):

    def __init__(self, control_stage_config, sd_locked=True, control_enc_locked=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.sd_locked = sd_locked
        self.control_enc_locked = control_enc_locked
        if self.sd_locked:
            for name, param in self.model.named_parameters():
                param.requires_grad = False
   
        self.control_model = instantiate_from_config(control_stage_config)
        if self.control_enc_locked:
    ...
    def randn_cropinput(self, lq, gt, ref, base_size=[64, 128, 256, 512]):
        cur_size_h = random.choice(base_size)
        cur_size_w = random.choice(base_size)
        init_h = lq.size(-2)//2
        init_w = lq.size(-1)//2
        lq = lq[:, :, init_h-cur_size_h//2:init_h+cur_size_h//2, init_w-cur_size_w//2:init_w+cur_size_w//2]
        gt = gt[:, :, init_h-cur_size_h//2:init_h+cur_size_h//2, init_w-cur_size_w//2:init_w+cur_size_w//2]
        ref = ref[:, :, init_h-cur_size_h//2:init_h+cur_size_h//2, init_w-cur_size_w//2:init_w+cur_size_w//2]
        assert lq.size(-1)>=64
        assert lq.size(-2)>=64
        return [lq, gt, ref]
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        age_cond = batch['age']     # -1 means ref is younger, 1 means ref is older
        ...
        if self.random_size:
            self.lq, self.gt, self.ref= self.randn_cropinput(self.lq, self.gt, self.ref)
        ...
        encoder_posterior_r = self.encode_first_stage(r)
        z_ref = self.get_first_stage_encoding(encoder_posterior_r).detach()
        ref_wo_enc = r.detach()
        ...
        # text_cond = ['a younger portrait of *' if cond == -1 else 'an older portrait of *' for cond in age_cond]
        text_cond = []
        for cond in age_cond:
            if cond[0] == 0:
                txt = 'a young portrait of *'
            elif cond[0] == 1:
                txt = 'a mid-aged portrait of *'
            else:
                txt = 'an old portrait of *'
            text_cond.append(txt)
        ...
        null_cond=['']
        while len(null_cond) < z.size(0):
            null_cond.append(null_cond[-1])
        if len(null_cond) > z.size(0):
            null_cond = null_cond[:z.size(0)]
        assert len(null_cond) == z.size(0)

        out = [z, [text_cond, null_cond]]
        out.append(z_gt)
        # out.append(z_ref)
        out.append(ref_wo_enc)

        if return_first_stage_outputs:
            # xrec = self.decode_first_stage(z_gt)
            out.extend([x, self.gt])
        # if return_first_stage_outputs:
        #     xrec = self.decode_first_stage(z_gt)
        #     out.extend([x, self.gt, xrec])
        # if return_original_cond:
    ...
    def apply_model(self, x_noisy, t, text_cond, null_cond, struct_cond, ref, return_ids=False):
        ...
        #     pass
        # else:
        #     if not isinstance(text_cond, list):
        #         text_cond = [text_cond]
        #     key = 'c_concat' if self.model.conditioning_key == 'concat' else 'c_crossattn'
        #     cond = {key: text_cond}
        ...
        # else:
        ref_control = self.control_model(x=x_noisy, hint=ref, timesteps=t, struct_cond=struct_cond, context=null_cond)

        x_recon = diffusion_model(x_noisy, t, ref_control=ref_control, context=text_cond, struct_cond=struct_cond)
        if isinstance(x_recon, tuple) and not return_ids:
            return x_recon[0]
        else:
            return x_recon
    ...
    def forward(self, x, c, gt, ref, *args, **kwargs):
        ...
        t_ori = torch.tensor([self.ori_timesteps[index_i] for index_i in index])
        t_ori = t_ori.long().to(x.device)
        text_cond, null_cond = c

        if self.model.conditioning_key is not None:
            assert text_cond is not None
            assert null_cond is not None
            if self.cond_stage_trainable:
                text_cond = self.get_learned_conditioning(text_cond)
                null_cond = self.get_learned_conditioning(null_cond)
            else:
                text_cond = self.cond_stage_model(text_cond)
                null_cond = self.cond_stage_model(null_cond)
            # if self.shorten_cond_schedule:  # TODO: drop this option
            #     print(s)
            #     tc = self.cond_ids[t].to(self.device)
            #     c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c.float()))
        if self.test_gt:
            ...
            struc_c = self.structcond_stage_model(x, t_ori)
        return self.p_losses(gt, text_cond, null_cond, struc_c, t, t_ori, x, ref, *args, **kwargs)

    # def on_after_backward(self):
    #     param_list_trainable = []
    #     param_list_untrainable = []
    #     for name, param in self.control_model.named_parameters():
    #         if param.requires_grad:
    #             param_list_trainable.append(name)
    #         else:
    #             param_list_untrainable.append(name)
    #         if param.grad is not None:
    #             print(f'Gradient for {name}: {param.grad.mean().item()}')
    #     print('>>>>>>>>>>>>>>>>control_model_trainable>>>>>>>>>>>>>>>>>>>>')
    #     print(param_list_trainable)
    #     print('>>>>>>>>>>>>>>>>control_model_untrainable>>>>>>>>>>>>>>>>>>>>')
    #     print(param_list_untrainable)

    
    def p_losses(self, x_start, text_cond, null_cond, struct_cond, t, t_ori, z_gt, ref, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        ...
        model_output = self.apply_model(x_noisy, t_ori, text_cond, null_cond, struct_cond, ref)
    ...
    @torch.no_grad()
    def progressive_denoising(self, text_cond, null_cond, ref, struct_cond, shape, verbose=True, callback=None, quantize_denoised=False,
                              img_callback=None, mask=None, x0=None, temperature=1., noise_dropout=0.,
                              score_corrector=None, corrector_kwargs=None, batch_size=None, x_T=None, start_T=None,
                              log_every_t=None):
        if not log_every_t:
        ...
        #     else:
        #         cond = [c[:batch_size] for c in cond] if isinstance(cond, list) else cond[:batch_size]
        text_cond = text_cond[:batch_size]
        null_cond = null_cond[:batch_size]
        ...
        for i in iterator:
            ...
            if self.shorten_cond_schedule:
                assert self.model.conditioning_key != 'hybrid'
                tc = self.cond_ids[ts].to(text_cond.device)
                text_cond = self.q_sample(x_start=text_cond, t=tc, noise=torch.randn_like(text_cond))
                null_cond = self.q_sample(x_start=null_cond, t=tc, noise=torch.randn_like(null_cond))

            img, x0_partial = self.p_sample(img, text_cond, null_cond, ref, struct_cond, ts,
                                            clip_denoised=self.clip_denoised,
                                            quantize_denoised=quantize_denoised, return_x0=True,
                                            temperature=temperature[i], noise_dropout=noise_dropout,
                                            score_corrector=score_corrector, corrector_kwargs=corrector_kwargs)
    ...
    @torch.no_grad()
    def sample(self, text_cond, null_cond, struct_cond, ref, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
               mask=None, x0=None, shape=None, time_replace=None, adain_fea=None, interfea_path=None, start_T=None,
               unconditional_conditioning=None,
               unconditional_guidance_scale=None,
        ...
        return self.p_sample_loop(text_cond,
                                  null_cond,
                                  ref,
                                  struct_cond,
                                  shape,
    ...
    @torch.no_grad()
    def p_sample_loop(self, text_cond, null_cond, ref, struct_cond, shape, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
                      log_every_t=None, time_replace=None, adain_fea=None, interfea_path=None,
                      unconditional_conditioning=None,
        ...
        for i in iterator:
            ...
            img = self.p_sample(img, text_cond, null_cond, ref, struct_cond_input, ts,
                                clip_denoised=self.clip_denoised,
                                quantize_denoised=quantize_denoised, t_replace=t_replace,
                                unconditional_conditioning=unconditional_conditioning,
                                unconditional_guidance_scale=unconditional_guidance_scale,
    ...
    @torch.no_grad()
    def p_sample(self, x, text_cond, null_cond, ref, struct_cond, t, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None,
                 unconditional_conditioning=None, unconditional_guidance_scale=None,
                 reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000]):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance(x=x, text_cond=text_cond, null_cond=null_cond, ref=ref, struct_cond=struct_cond, t=t, clip_denoised=clip_denoised,
                                       return_codebook_ids=return_codebook_ids,
                                       quantize_denoised=quantize_denoised,
                                       return_x0=return_x0,
                                       score_corrector=score_corrector, corrector_kwargs=corrector_kwargs, t_replace=t_replace,
    ...
    def p_mean_variance(self, x, text_cond, null_cond, ref, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None, t_replace=None, unconditional_conditioning=None, unconditional_guidance_scale=None,
                        reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000]):
        if t_replace is None:
            t_in = t
        ...
        model_out = self.apply_model(x, t_in, text_cond, null_cond, struct_cond, ref, return_ids=return_codebook_ids)
        # if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
        #     model_out = self.apply_model(x, t_in, text_cond, null_cond, struct_cond, ref, return_ids=return_codebook_ids)
        # else:
        #     x_in = torch.cat([x] * 2)
    ...
    @torch.no_grad()
    def sample_log(self,text_cond, null_cond, ref, struct_cond, batch_size, ddim, ddim_steps, **kwargs):
        ...
        if ddim:
            ...
            samples, intermediates = self.sample(text_cond=text_cond, null_cond=null_cond, struct_cond=struct_cond, ref=ref, batch_size=batch_size,
                                                 return_intermediates=True, **kwargs)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        log = dict()
        z, c_lq, z_gt, ref_wo_enc, x, gt = self.get_input(batch, self.first_stage_key,
                                           return_first_stage_outputs=True,
                                           force_c_encode=True,
                                           return_original_cond=True,
                                           bs=N, val=True)
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                ...
                samples, z_denoise_row = self.sample(text_cond=text_cond, null_cond=null_cond, struct_cond=struct_cond, ref=ref_wo_enc, batch_size=N, timesteps=cur_time_step, return_intermediates=True, time_replace=self.time_replace)
            x_samples = self.decode_first_stage(samples)
            log["samples"] = x_samples
            if plot_denoise_rows:
                denoise_grid = self._get_denoise_row_from_list(z_denoise_row)
            ...
            if quantize_denoised and not isinstance(self.first_stage_model, AutoencoderKL) and not isinstance(
                    self.first_stage_model, IdentityFirstStage):
                with self.ema_scope("Plotting Quantized Denoised"):
                    samples, z_denoise_row = self.sample_log(text_cond=text_cond, null_cond=null_cond, ref=ref_wo_enc, struct_cond=struct_cond,batch_size=N,ddim=use_ddim,
                                                             ddim_steps=ddim_steps,eta=ddim_eta,
                                                             quantize_denoised=True, x_T=x_T)
                x_samples = self.decode_first_stage(samples.to(self.device))
                log["samples_x0_quantized"] = x_samples
        ...
        if plot_progressive_rows:
            with self.ema_scope("Plotting Progressives"):
                img, progressives = self.progressive_denoising(text_cond, null_cond, ref_wo_enc, struct_cond=struct_cond,
                                                               shape=(self.channels, self.image_size//8, self.image_size//8),
                                                               batch_size=N)
            prog_row = self._get_denoise_row_from_list(progressives, desc="Progressive Generation")
            log["progressive_row"] = prog_row
    ...
    def configure_optimizers(self):
        ...
        params = params + list(self.cond_stage_model.parameters())
        params = params + list(self.structcond_stage_model.parameters())
        params = params + list(self.control_model.parameters())
        if self.learn_logvar:
            assert not self.learn_logvar
            print('Diffusion model optimizing logvar')
            params.append(self.logvar)
